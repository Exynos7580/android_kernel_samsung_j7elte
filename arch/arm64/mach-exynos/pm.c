...
#define EXYNOS7580_EINT_PEND(b, x)	((b) + 0xA00 + (((x) >> 3) * 4))
...
static void exynos_show_wakeup_reason_eint(void)
{
	...
	for (i = 0, size = 8; i < 32; i += size) {
		...
		ext_int_pend =
			__raw_readl(EXYNOS7580_EINT_PEND(exynos_eint_base, i));
...
/*
	if (!soc_is_exynos7580() || (samsung_rev() != EXYNOS7580_REV_0))
...
#ifdef CONFIG_SOC_EXYNOS7580
	exynos_eint_base = ioremap(EXYNOS7580_PA_GPIO_ALIVE, SZ_8K);
