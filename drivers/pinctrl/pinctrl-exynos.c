...
int exynos7580_secgpio_get_nr_gpio(void)
{
	int i, j;
	int nr_gpio = 0;

	for (i = 0; i < ARRAY_SIZE(exynos7580_pin_ctrl); i++) {
		for (j = 0; j < exynos7580_pin_ctrl[i].nr_banks; j++)
			nr_gpio += exynos7580_pin_ctrl[i].pin_banks[j].nr_pins;
	}
...
u32 exynos_eint_to_pin_num(int eint)
{
	return exynos7580_pin_ctrl[0].base + eint;
}
#endif
