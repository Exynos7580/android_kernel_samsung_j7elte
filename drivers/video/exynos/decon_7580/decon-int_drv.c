...
void decon_int_set_clocks(struct decon_device *decon)
{
	...
	decon_clk_set_rate(dev, disp_pll, decon->pdata->disp_pll_clk);

	/* eclk: decon core clock */
	decon_clk_set_parent(dev, mout_sclk_disp_decon_int_eclk_a, "bus_pll");
	decon_clk_set_parent(dev, mout_sclk_disp_decon_int_eclk_b, "mout_sclk_disp_decon_int_eclk_a");
	decon_clk_set_parent(dev, mout_sclk_decon_int_eclk_user, "sclk_disp_decon_int_eclk");
	decon_clk_set_parent(dev, m_decon0_eclk, "mout_sclk_decon_int_eclk_user");
	decon_clk_set_divide(dev, dout_sclk_disp_decon_int_eclk, 4);
	decon_clk_set_divide(dev, d_decon0_eclk, 0);
	/* vclk: pixel clock */
	decon_clk_set_parent(dev, m_decon0_vclk, "disp_pll");
	decon_clk_set_divide(dev, d_decon0_vclk, 0);

	/* aclk_xiu_disp_rw */
	decon_clk_set_parent(dev, mout_aclk_disp_200, "bus_pll");
	decon_clk_set_divide(dev, dout_aclk_disp_200, 3);
	decon_clk_set_parent(dev, mout_aclk_disp_200_user, "aclk_disp_200");

	/* pclk: bus clock, aclk: DMA operation clock and SFR access */
	decon_clk_set_divide(dev, d_pclk_disp, 1);
}
...
int decon_mmap(struct fb_info *info, struct vm_area_struct *vma)
{
	...
	printk(KERN_INFO " %s : before: %d  %p %p %p \n ",__func__, ret, vma, win->dma_buf_data[0].dma_buf, &win->dma_buf_data[0]);
