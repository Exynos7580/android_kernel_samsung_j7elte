...
static unsigned int max_deep_buffer_size = MAX_DEEPBUF_SIZE;
	...
	struct snd_pcm_hw_params *params)
{
	...
	if ((substream->stream == SNDRV_PCM_STREAM_CAPTURE) &&
		...
		(totbytes > max_deep_buffer_size) && dram_uhqa_tx_buf)
					...
					struct device_node *np)
{
	...
	/*
	 * samsung,tx-deepbuf-size is provided, then use the given value,
	 * else default preinitilised value will be used, so don't exit with error.
	 */
	if (of_property_read_u32(np, "samsung,tx-deepbuf-size", &val))
		pr_info("%s: Failed to get tx-deepbuf-size \n", __func__);
	else
		max_deep_buffer_size  = (size_t)val;
	...
	if (!dram_uhqa_tx_buf) {
		if (of_find_property(np, "samsung,tx-uhqa-buf", NULL)) {
			...
			dram_uhqa_tx_buf->area = dma_alloc_coherent(pcm->card->dev,
					dram_uhqa_tx_buf->bytes,
					&dram_uhqa_tx_buf->addr, GFP_KERNEL);
